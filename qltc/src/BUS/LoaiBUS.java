...
public class LoaiBUS {
	...
	private ArrayList<LoaiDTO> listloai = new ArrayList<LoaiDTO>();

	public ArrayList<LoaiDTO> getLoaisachLoai() {

		return LoaiSachDAL.getdanhsachLoai();
	}

	public int themloai(LoaiDTO loai) {
		if(loai.getTenloai().isEmpty()) {
			JOptionPane.showMessageDialog(null, "Không được để trống tên loại");
			return -1 ;
		}
		for (LoaiDTO loai2 : getLoaisachLoai()) {
	...
	public int xoaloai(LoaiDTO loai) {
	...
	public int sualoai(LoaiDTO loai) {
		if(loai.getTenloai().isEmpty()) {
			JOptionPane.showMessageDialog(null, "Không được để trống tên loại");
			return -1 ;
		}
