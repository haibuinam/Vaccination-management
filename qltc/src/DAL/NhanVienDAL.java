...
public class NhanVienDAL {
	public static int themnhanvien(NhanVien loai) {
		int i = -1;
		String sql = "insert into nhanvien (tennv,namsinh,gioitinh,diachi,sdt,donvitiem,trangthai) values(?,?,?,?,?,?,?)";
		try {
			...
			/////
			pstm.setString(6, loai.getDonViTiem());
			/////
			pstm.setInt(7, 1);
	...
	public static int suanhanvien(NhanVien loai) {
		int i = -1;
		String sql = "update nhanvien set tennv = ?, namsinh = ?, gioitinh = ?, diachi =?, sdt = ?, donvitiem = ? where manv = ?";
	...
	public static ArrayList<NhanVien> getdanhsachnv() {
		try {
			...
			while (rs.next()) {
				...
				//////////
				nhanVien.setDonViTiem(rs.getString(8));
				////////
				dsl.add(nhanVien);
