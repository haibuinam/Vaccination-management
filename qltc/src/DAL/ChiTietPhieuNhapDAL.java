...
public class ChiTietPhieuNhapDAL {
	public static ArrayList<ChiTietPhieuNhap> getdanhsachctphieunhap() {
		try {
			...
			while (rs.next()) {
				...
				tg.setMaSach(rs.getInt("MaVaccine"));
	...
	public static int Themctpn(ChiTietPhieuNhap ke) {
		int i = -1;
		String sql = "insert into chitietphieunhap (MaPN,MaVaccine,Gia,SoLuong,ThanhTien) values(?,?,?,?,?)";
		...
		try {
			...
			if (i > 0) {
				...
				stm2.setInt(1, VaccineDAL.getsoluongsach(ke.getMaSach()) + ke.getSoLuong());
				stm2.setInt(2, ke.getMaSach());
				stm2.executeUpdate();
				System.out.println("Cập nhật số lượng MaVaccine sau khi nhập thành công");
			}
			conn.close();
	...
	public static int Suactpn(ChiTietPhieuNhap ke) {
		int i = -1;
		String sql = "update chitietphieunhap set MaPN = ? , MaVaccine = ?, Gia = ?  ,SoLuong = ? , ThanhTien=?  where MaCTPN = ?";
	...
	public static ArrayList<ChiTietPhieuNhap> getthongke() {
		...
		try {
			String sql = "SELECT chitietphieunhap.*, phieunhap.NgayNhap, vaccine.TenVaccine FROM chitietphieunhap INNER JOIN phieunhap ON chitietphieunhap.MaPN = phieunhap.MaPhieuNhap INNER JOIN vaccine ON chitietphieunhap.MaVaccine = vaccine.MaVaccine";
			Connection conn = DBConnect.getConnection();
			...
			while (rs.next()) {
				...
				ct.setMaSach(rs.getInt("MaVaccine"));
				ct.setGia(rs.getInt("Gia"));
				ct.setSoLuong(rs.getInt("Soluong"));
				ct.setThanhTien(rs.getInt("ThanhTien"));
				ct.setTensach(rs.getString("TenVaccine"));
	...
	public static ArrayList<ChiTietPhieuNhap> getthongketheonam(int nam) {
		ArrayList<ChiTietPhieuNhap> list = new ArrayList<ChiTietPhieuNhap>();
		try {
			String sql = "SELECT chitietphieunhap.*, phieunhap.NgayNhap, vaccine.TenVaccine FROM chitietphieunhap INNER JOIN phieunhap ON chitietphieunhap.MaPN = phieunhap.MaPhieuNhap INNER JOIN vaccine ON chitietphieunhap.MaVaccine = vaccine.MaVaccine WHERE phieunhap.NgayNhap BETWEEN '"
	...
	public static ArrayList<ChiTietPhieuNhap> getthongketheongay(String date1, String date2) {
		ArrayList<ChiTietPhieuNhap> list = new ArrayList<ChiTietPhieuNhap>();
		try {
			String sql = "SELECT chitietphieunhap.*, phieunhap.NgayNhap, sach.TenVaccine FROM chitietphieunhap INNER JOIN phieunhap ON chitietphieunhap.MaPN = phieunhap.MaPhieuNhap INNER JOIN vaccine ON chitietphieunhap.MaVaccine = vaccine.MaVaccine WHERE phieunhap.NgayNhap BETWEEN '"
					+ date1 + "' AND '" + date2 + "'";
			Connection conn = DBConnect.getConnection();
			Statement stm = conn.createStatement();
			ResultSet rs = stm.executeQuery(sql);
